...
> **NOTE** Solutions for all questions are already in Flights.java file, please don't look at it :-). Try to come up with your solution first. We can check the file later.
___
...
#### 6. Build/submit/run (optional)
  When your solutions return right answers try write them out to output directory ```/root/workshop-spark/data/task2/output```. You can try to gather all your code into Flights.java, build and submit it with the command bellow.
  ```
  spark-submit \
